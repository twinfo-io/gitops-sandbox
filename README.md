...
- **Check semântico anti-alucinação:** para `agent:generate-code` e `agent:generate-tests`, um segundo LLM-judge compara a spec da issue (intenção) contra o diff real (implementação) — método intended-vs-implemented. Não bloqueia o merge (LLM-judge pode ter falso positivo); vira comentário extra no Linear pra dar contexto pro humano que vai aprovar
...
- **Branch protection em `main`** (TWI-349/E14): status check obrigatório (`Eval Prompts (static)`) + `enforce_admins`, sem bypass mesmo pra owner do repo. **Sem** aprovação humana obrigatória — testado e revertido, já que o repo é solo na prática e GitHub nunca permite auto-aprovação (travaria todo merge). Ver `docs/GOVERNANCE.md`. PRs de agents (`agent:generate-code`/`agent:generate-tests`) seguem exigindo merge manual do humano; só os PRs internos do `gitops-sync` (sync de demanda/épico) usam `gh pr merge --auto --squash`, já que são mudanças puramente estruturais no Linear
