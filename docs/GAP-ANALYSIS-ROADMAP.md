...
| E9 | Isolamento de subtarefa com ledger de progresso em arquivo (subagent-driven-development) | Médio | Alto | Tarefas grandes falham monoliticamente sem checkpoint | **P3** | ✅ Done (versão enxuta — ledger em `.gitops/ledger/`, sem reestruturar em subagents isolados) | [TWI-302](https://linear.app/twinfo-lifters/issue/TWI-302) | — |
| E10 | Geração automática de testes a partir do diff (padrão Keploy) | Médio | Médio | Cobertura de teste não cresce junto com código gerado por agente | **P3** | ✅ Done (versão enxuta — `agent:suggest-tests` só sugere via comentário, não gera/commita) | [TWI-303](https://linear.app/twinfo-lifters/issue/TWI-303) | — |
...
| E12 | Skills de plugin (pm-skills) nunca instaladas no ambiente de CI — `/create-prd`, `skill:*` falhariam silenciosamente | Alto | Baixo | Agent roda mas ignora a skill, produz output degradado sem avisar | **P1** | ✅ Done | [TWI-323](https://linear.app/twinfo-lifters/issue/TWI-323) | — |
